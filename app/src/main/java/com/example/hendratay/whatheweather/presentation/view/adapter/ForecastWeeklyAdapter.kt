...
class ForecastWeeklyAdapter(val forecastList: List<ListItem>):
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when(holder.itemViewType) {
            ListItem.TYPE_DATE -> {
                var dateItem: DateItem = forecastList[position] as DateItem
                var dateViewHolder: DateViewHolder = holder as DateViewHolder
                dateViewHolder.dateTextview.text = dateItem.date
            }
            ListItem.TYPE_GENERAL -> {
                var generalItem: GeneralItem = forecastList[position] as GeneralItem
                ...
                generalViewHolder.tempTextView.text = generalItem.forecastView.main.temp.toString() + "\u2103"
            }
        }
